...
--테이블 수준의 제약조건만 설정 가능
--부모테이블의 PRINARY KEY 제약조건이 설정된 컬럼을 참조하여 자식테이블의 컬럼에 FOREIGN KEY 제약조건 설정
--테이블의 관계를 구현하기 위한 제약조건 

--SUBJECT1 테이블 생성 - 과목코드(숫자형-PRIMARY KEY), 과목명(문자형) : 부모테이블
...
INSERT INTO SUBJECT1 VALUES(20,'JAP');
...
--결합조건 : TRAINEE1테이블의 수강과목코드(SCODE)와 SUBJECT1 테이블의 과목코드(SNO)가 같은 행을 결합 -INNER JOIN
--INNER JOIN은 결합조건이 참(TRUE)인 행만 검색하므로 결합조건이 맞지 않는 행은 미검색된다
...
--TRAINEE2 테이블 생성 - 수강생번호(숫자형-PRIMARY KEY), 수강생이름(문자형), 수강과목코드(숫자형-FOREIGN KEY) : 자식테이블
--자식테이블의 컬럼에 설정된 FOREIGN KEY 제약조건은 부모 테이블의 PRIMARY KEY 제약조건이 설정된 컬럼 참조
...
--자식테이블에서 참조되는 부모테이블의 행 삭제 불가능
DELETE FROM SUBJECT1 WHERE SNO=10; --FOREIGN KEY 제약조건을 위반하여 에러 발생


--SUBJECT2 테이블 생성 - 과목코드(숫자형-PRIMARY KEY), 과목명(문자형) : 부모테이블
...
INSERT INTO SUBJECT2 VALUES(20,'JAP');
INSERT INTO SUBJECT2 VALUES(30,'SPRING');
SELECT * FROM SUBJECT2;
COMMIT;


--TRAINEE3 테이블 생성 - 수강생번호(숫자형-PRIMARY KEY), 수강생이름(문자형), 수강과목코드(숫자형-FOREIGN KEY) : 자식테이블
--TRAINEE3 테이블의 수강과목코드(SCODE)에 FOREIGN KEY 제약조건을 설정하여 SUBJECT2 테이블의 과목코드(SNO)를 참조
--FOREIGN KEY 제약조건 설정시 ON DELETE CASCADE 또는 ON DELETE SEL NULL 기능 추가
-->ON DELETE CASCADE : 부모테이블의 행을 삭제할 경우 자식 테이블의 참조 컬럼값이 저장된 행도 삭제 처리하는 기능 제공
-->ON DELETE SEL NULL : 부모테이블의 행을 삭제할 경우 자식 테이블의 참조 컬럼값을 NULL로 변경하는 기능 제공
...
--SUBJECT2 테이블(부모테이블)의 과목코드를 참조하는 TRAINEE3 테이블의 수강생정보 삭제
SELECT * FROM SUBJECT2;
SELECT * FROM TRAINEE3;

--서브쿼리를 사용하여 테이블 생성 가능 - 기존 테이븥을 이용하여 새로운 테이블 생성 : 행 복사
--형식)CREATE TABLE 타겟테이블명[(컬럼명, 컬럼명,...)] AS SELECT 검색대상, 검색대상,... FROM 원본테이블명 [WHERE 조건식]
--서브쿼리의 검색결과를 사용하여 타겟테이블을 생성하고 검색된 행은 타겟테이블의 행으로 삽입처리
...
--오라클 휴지통 확인
--오라쿨 휴지통에는 테이블뿐만 아니라 테이블과 종속관계에 있는 인덱스(INDEX) 객체도 같이 존재 
SHOW RECYCLEBIN;
...
--MGR1테이블 삭제
SELECT TABLE_NAME FROM USER_TABLES WHERE TABLE_NAME LIKE 'MGR1%';
...
SELECT * FROM BONUS;
ROLLBACK; --롤백처리 : 트렌젝션 초기화 - 트렌젝션에 저장된 모든 AQL 명령 제거
SELECT * FROM BONUS;

--BONUS  테이블 초기화
TRUNCATE TABLE BONUS; --자동 커밋처리
SELECT * FROM BONUS;
ROLLBACK;--롤백 불가능
...
CREATE VIEW EMP_VIEW10 AS SELECT EMPNO,ENAME,SAL,DNAME FROM EMP JOIN DEPT ON EMP.DEPTNO=DEPT.DEPTNO; --에러발생
--기존뷰를 새로운 뷰로 대체 - 뷰 변경
CREATE OR REPLACE VIEW EMP_VIEW10 AS SELECT EMPNO,ENAME,SAL,DNAME FROM EMP JOIN DEPT ON EMP.DEPTNO=DEPT.DEPTNO;
SELECT VIEW_NAME,TEXT FROM USER_VIEWS;

--뷰를 생성하지 않고 SELECT 명령의 서브쿼리를 사용하여 인라인뷰를 생성하여 사용 - CREATE VIEW 시스템 권한이 없어도 사용 가능
