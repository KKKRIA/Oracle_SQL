...
SELECT EMPNO,ENAME,SAL FROM (SELECT EMPNO,ENAME,SAL FROM EMP); --괄호안 -> 가상의 테이블이 일시적으로 생성
...
--행번호(ROWNUM)가 6보다 작은 행 검색
SELECT ROWNUM, TEMP.* FROM (SELECT EMPNO,ENAME,SAL FROM EMP) TEMP WHERE ROWNUM<6;

--행번호(ROWNUM)가 5인 행 검색 
--ROWNUM 키워드로 행번호를 제공받아 조건식에서 비교할 경우 <(<=)연산자를 이용한 검색은 가능하지만 = 또는 >(>=) 연산자를 이용한 검색 불가능 - 순차적으로 제공
SELECT ROWNUM, TEMP.* FROM (SELECT EMPNO,ENAME,SAL FROM EMP) TEMP WHERE ROWNUM=5; -- 검색오류
...
--EMP 테이블에 저장된 모든 사원의 사원번호,사원이름,급여를 /급여순으로 내림차순 정렬/하여 검색
--검색된 행에 ROWNUM 키워드로 행번호를 제공받아 검색 
SELECT ROWNUM,EMPNO,ENAME,SAL FROM EMP ORDER BY SAL DESC;--검색 오류 --행번호가 붙은 뒤 SAL 정렬됐음- 뒤죽박죽 
...
--EMP 테이블에 저장된 모든 사원 중 급여를 6번째부터 10번째까지 많이 받은 사원의 사원번호,사원이름,급여 검색  내림차순
...
SELECT NAME,0 FROM SUPER_HERO UNION SELECT NAME,GRADE FROM MARVEL_HERO; --임의 값 0사용
SELECT NAME,NULL FROM SUPER_HERO UNION SELECT NAME,GRADE FROM MARVEL_HERO; --임의값  NULL사용
...
SELECT * FROM DEPT; --> 원하는 값이 제대로 들어갔는지 확인
COMMIT; --> 등록, 돌이킬 수 없음
