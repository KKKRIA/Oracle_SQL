...
--현재 세션에서 작업중인 테이블의 행을 커밋처리 전까지 다른 세션에서 검색되지 않고 기존행이 검색되도록 트렌젝션 사용 
--데이터 일관성을 제공하기 위해 트렌젝션 사용
...
--커밋 처리를 이용해 트렌젝션에 저장된 SQL 명령을 실제 테이블에 적용하면 다른 세션에서도 테이블의 조작된 행이 검색
COMMIT;

--데이타 잠금 기능(LOCK)을 제공하기 위해 트렌젝션 사용
--DBMS 프로그램은 다중 사용자 환경으로 같은 테이블의 행을 다른 세션에서 조작 가능
--현재 세션에서 작업중인 테이블의 행을 다른 세션에서 작업하지 못하도록 트렌젝션을 사용하여 데이타 잠금기능 제공

--BONUS 테이블에서 사원이름이 ALLEN 인 사원의 급여를 2000으로 변경
SELECT * FROM BONUS;
UPDATE BONUS SET SAL =2000 WHERE ENAME='ALLEN'; --테이블 행에 대한 데이타 잠금기능 활성화 --다른 세션에서 행을 변경처리 할 수 없음 -- 현재 세션에서 잠금이 활성화 되어있기 때문
SELECT * FROM BONUS;

--다른 세션을 사용하여 BONUS 테이블에서 사원이름이 ALLEN인 사원의 성과급을 급여의 50%로 변경
--UPDATE BONUS SET COMM=SAL*0.5 WHERE ENAME='ALLEN'
--현재 세션에서 작업중인 테이블의 행을 다른 세션에서 조작 할 경우 트렌젝션의 데이타 잠금기능으로 인해 다른 세션이 일시적으로 중지
--현재 세션에서 작업중인 테이블의 행에 대한 트렌젝션의 명령을 커밋 또는 롤백 처리해야만 데이타 잠금 기능이 비활성화 되어 다른 세션의 DML명령이 실행 가능
COMMIT;

--■ SAVEPOINT ■: 트렌젝션에 라벨(위치정보)을 부여하는 명령
--트렌젝션에 저장된 DML명령 중 라벨을 이용하여 원하는 위치의 DML 명령만 롤백처리하기 위해 SAVEPOINT 사용
--형식)SAVEPOINT 라벨명
...
DELETE FROM BONUS WHERE ENAME='ALLEN';
SELECT * FROM BONUS;
SAVEPOINT ALLEN_DELETE_AFTER; --ALLEN_DELETE_AFTER 이건 라벨명 이름 맘대로 설정가능 -- 트렌젝션에 라벨을 생성하여 부착
...
--데이타베이스의 객체(테이블, 뷰, 시퀀스, 인덱스, 동의어, 사용자 등)를 관리하기 위한 SQL 명령
...
--3. 영문자 외에 다른 문자 (한글 등) 사용 가능 - 비권장
--4. 키워드로 식별자를 선언할 경우 에러 발생 - " " 안에 표현하면 가능하지만 비권장

--■ 자료형(DATATYPE) ■: 컬럼에 저장 가능한 값의 형태를 표현하기 위한 키워드
--1. 숫자형 : NUMBER[(전체자릿수, 소숫점자릿수)] -> 대괄호는 생략가능

--2. 문자형 : 
-->> CHAR(크기) - 크기 : 1~2000(BYTE) >> 고정길이      
-->> VARCHAR2(크기) - 크기 : 1~4000(BYTE) >> 가변길이 - 저장된 값에 따라 크기가 바뀜 - 더 많이 사용됨 ??       
-->> LONG - 최대 2GBYTE 저장 >> 가변길이 - 테이블에 하나의 컬럼에만 설정 가능하며 정렬 불가능
...
--USER_TABLES : 현재 접속 사용자의 스키마로 접근 가능한 테이블 정보를 제공하는 딕셔너리
SELECT TABLE_NAME FROM USER_TABLES;
SELECT * FROM USER_TABLES; --> 전체정보가 나옴
...
INSERT INTO SAWON1 VALUES(2000,'홍길동',800000);
SELECT * FROM SAWON1;
COMMIT;
...
--■ NOT NULL ■: NULL을 허용하지 않는 제약 조건 - 컬럼에 반드시 값ㅇ르 저장되도록 설정하는 제약조건
...
INSERT INTO USER2 VALUES('IJQ','일지매',NULL);
SELECT * FROM USER2;
COMMIT;
...
INSERT INTO USER4 VALUES('ABC','임꺽정','010-7890-1234');--아이디 중복 삽입가능
INSERT INTO USER4 VALUES('XYZ','임꺽정','010-1234-5678');--전화번호 중복 삽입가능
SELECT * FROM USER4;
COMMIT;
