...
--형식)FOR INDEX_COUNTER IN [REVERSE] LOWER_BOUND..HIGH_BOUND LOOP 명령; 명령; ... END LOOP;  //[REVERSE] 감소한다는 뜻
...
--■ 저장함수(STORED FUNCTION) ■: 저장 프로시저와 유사한 기능을 제공하지만 반드시 하나의 결과값을 반환
--형식) CREATE [OR REPLACE] PROCEDURE 저장함수명[(매개변수 [MODE] 자료형, 매개변수 [MODE]자료형,...)]
...
SELECT NAME, TEXT FROM USER_SOUCE WHERE NAME='CAL_SAL';

--저장 함수의 반환값을 저장하기 위한 바인딩 변수 선언
VARIABLE VAR_SAL NUMBER;

--저장함수 호출 - 저장 함수의 반환값을 바인딩 변수에 저장
EXECUTE :VAL_SAL := CAL_SAL(7788);
...
--FOR EACH ROW : 생략된 경우 문장 레벨의 트리거를 생성하고 선언한 경우 행 레벨 트리거로 생성
--문장 레벨의 트리거 : 이벤트 DML 명령이 실행되면 트리거에 작성된 PL/SQL 프로시저의 명령을 한번만 실행
--행 레벨의 트리거 : 이벤트 DML 명령이 실행되면 트리거에 작성된 PL/SQL 프로시저의 명령을 행의 갯수만큼 실행
