...
--컬럼에 인덱스를 설정하면 인덱스 영역을 생성하여 검색관련 정보를 저장해 컬럼에 대한 행 검색 속도 증가
--조건식에서 많이 사용하는 컬럼에 설명하며 행이 많을 때 인덱스를 설정하는것이 효율적
...
--COMM 테이블에 대한 저뇽 동의어로 BONUS 생성
SELECT * FROM COMM;
SELECT * FROM BONUS; -- 테이블 또는 뷰가 없으므로 에러 발생
--CREATE SYNONYM 시스템 권한이 없으므로 CREATE SYNONYM 명령을 실행하면 에러 발생
CREATE SYNONYM BONUS FOR COMM; --권한이 불충분하여 에러발생

--시스템 관리자(SYSDBA - SYS계정)로 접속하여 현재 접속 사용자(SOCTT 계정)에서 CREATE SYNONYM 시스템 권한을 부여
--GRANT CREATE SYNONYM TO SCOTT;

--시스템 관리자에게 CREATE SYNONYM 시스템 권한을 부여 후 CREATE SYNONYM 명령 실행
...
--■ 사용자(USER) ■: 시스템(DBMS)를 사용할 수 있는 객체 - 계정(ACCOUNT): 권한을 가지고 있는 사용자
...
--ALTER SESSION SET "_ORACLE_SCRIPT"=TRUE; --SYS ADMIN에 작성
--CREATE USER KIM IDENTIFIED BY 1234; --SYS ADMIN에 작성
...
--형식)ALLTER USER 계정명 INENTIFIED BY 비밀번호

--KIM 계정의 비밀번호 변경
--ALTER USER KIM IDENTIFIED BY 5678;

--계정 상태 변경 - OPEN(계정 활성화 - DBMS 서버 접속 가능), LOCK(계정 비활성화)
...
--계정에게 권한을 부여하거나 회수하는 SQL명령
--오라클 권한(PRACLE PRIVILEGE): 시스템 권한, 객체 권한
...
--SQL>CREATE TABLE SAWON(NI NUMBER(4)PRIMARY KEY,NAME VARCHAR2(20),PAY NUMBER ); --권한이 불충분하여 에러 발생

-->> 시스템관리자(SYSDBA - SYS 계정)가 KIM 계정에게 CREATE TABLE 시스템 관한 부여
...
--오라클에서 기본적으로 제공되는 롤 - COMNECT, RESOURCE, DBA 등
...
--오라클 서버에 LEE 계정으로 접속하여 SAWON 테이블 생성 - SQLPLUS 이용
...
--■ PL/SQL(PROCEDURAL LANGUAGE EXTENSION SQL) ■: SQL에 없는 변수 선언, 선택처리, 반복 처리를 제공하는 절차적인 언어

--세부분의 영역으로 구분하여 PL/SQL 작성
--1. DECLARE 영역(선언부) : DECLARE - 변수를 선언하는 영역(선택)
--2. EXECUTERBLE 영역(실행부) : BEGIN -SQL 명령을 포함한 다수의 명령을 작성하는 영역(필수)
--3. EXCEPTION 영역(예외처리부) : EXCEPTION - 예외를 처리하기 위한 명령을 작성하는 영역(선택)

--영역에서 하나의 명령을 구분하기 위해 ; 사용
--마지막 영역은 EMD 키워드로 마무리 후 ; 사용
--PL/SQL 실행을 위해서는 마지막에 / 기호 사용
...
--★ ■ 레퍼런스 변수 ■: 다른 변수의 자료형 또는 테이블의 컬럼 자료형을 참조하여 선언된 변수 - 선언부 ★자주사용
...
BEGIN
    ...
    /* 단일행을 검색하여 검색행의 컬럼값을 레퍼런스 변수에 저장 - 다중행이 검색될 경우 에러발생*/
...
--■ 테이블 변수 ■: 테이블의 저장된 행을 여러 개 검색하여 얻은 다수의 컬럼값을 저장하기 위해 선언하는 변수 - 배열
--형식)테이블변수명 테이블타입명
...
--테이블 변수는 테이블 변수의 요소를 첨자로 구분하여 사용 - 첨자는 1부터 1씩 증가된 숫자값
--형식)테이블변수명(첨자)

--EMP 테이블의 EMPNO 컬럼과 ENAME 컬럼을 참조하여 테이블 변수를 선언하고 EMP 테이블에 저장된 모든 사원의 사원번호와 사원이름을 검색하여 테이블 변수에 저장하여 출력하는 PL/SQL 작성
DECLARE
    /*테이블 타입 선언*/
    ...
    I BINARY_INTEGER :=0;
BEGIN
    /*EMP 테이블에 저장된 모드 사원의 사원번호, 사원이름을 검색하여 테이블 변수의 요소에 저장하기 위한 반복문*/
...
--★ ■ 레코드 변수 ■: 테이블에 저장된 하나의 행의 모든 컬럼값을 저장하기 위해 선언하는 변수 ★자주사용
--형식)레코드변수명 레코드타입명
...
--사원번호가 7844인/ 사원의 사원번호, 사원이름, 업무, 급여,부서번호를 검색/하여 레코드 변수에 저장 후 출력하는 PL/SQL 작성
DECLARE
    /* 레코드 타입 선언 */
